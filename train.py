        ...
        for idx, data in enumerate(dataset, start=epoch_iter):
            ...
            if opt.sparse_D:
                real_B_all, fake_B_all, flow_ref_all, conf_ref_all = [
                    None]*t_scales, [None]*t_scales, [None]*t_scales, [None]*t_scales
            # temporally subsampled frames
            real_B_skipped, fake_B_skipped = [None]*t_scales, [None]*t_scales
            flow_ref_skipped, conf_ref_skipped = [
                None]*t_scales, [None]*t_scales       # temporally subsampled flows

            for i in range(0, n_frames_total-t_len+1, n_frames_load):
                # 5D tensor: batchSize, # of frames, # of channels, height, width
                input_A = Variable(
                    data['A'][:, i*input_nc:(i+t_len)*input_nc, ...]).view(-1, t_len, input_nc, height, width)
                input_B = Variable(
                    data['B'][:, i*output_nc:(i+t_len)*output_nc, ...]).view(-1, t_len, output_nc, height, width)
                inst_A = Variable(data['inst'][:, i:i+t_len, ...]).view(-1, t_len,
                                                                        1, height, width) if len(data['inst'].size()) > 2 else None
                ...
                fake_B_prev = real_B_prev[:,
                                          0:1] if fake_B_last is None else fake_B_last[0][:, -1:]
                if fake_B.size()[1] > 1:
                    fake_B_prev = torch.cat(
                        [fake_B_prev, fake_B[:, :-1].detach()], dim=1)

                losses = modelD(0, reshape(
                    [real_B, fake_B, fake_B_raw, real_A, real_B_prev, fake_B_prev, flow, weight, flow_ref, conf_ref]))
                losses = [torch.mean(
                    x) if x is not None else 0 for x in losses]
                ...
                if t_scales > 0:
                    if opt.sparse_D:
                        ...
                        flow_ref_all, flow_ref_skipped = get_skipped_frames_sparse(
                            flow_ref_all, flow_ref, t_scales, tD, n_frames_load, i, is_flow=True)
                        conf_ref_all, conf_ref_skipped = get_skipped_frames_sparse(
                            conf_ref_all, conf_ref, t_scales, tD, n_frames_load, i, is_flow=True)
                    else:
                ...
                for s in range(t_scales):
                    if real_B_skipped[s] is not None:
                        losses = modelD(
                            s+1, [real_B_skipped[s], fake_B_skipped[s], flow_ref_skipped[s], conf_ref_skipped[s]])
                ...
                for s in range(t_scales_act):
                    ...
                    loss_D_T.append(
                        (loss_dict_T[s]['D_T_fake'] + loss_dict_T[s]['D_T_real']) * 0.5)
                ...
                # temporal discriminator
                for s in range(t_scales_act):
                    optimizer_D_T = getattr(
                        modelD.module, 'optimizer_D_T'+str(s))
            ...
            if total_steps % opt.print_freq == 0:
                ...
                for s in range(len(loss_dict_T)):
                    errors.update({k+str(s): v.data.item() if not isinstance(v,
                                                                             int) else v for k, v in loss_dict_T[s].items()})
            ...
            if save_fake:
                if opt.label_nc != 0:
                    input_image = util.tensor2label(
                        real_A[0, -1], opt.label_nc)
                    ...
                    if real_A.size()[2] == 6:
                        ...
                        input_image[input_image2 !=
                                    0] = input_image2[input_image2 != 0]
                else:
                    c = 3 if opt.input_nc == 3 else 1
                    input_image = util.tensor2im(
                        real_A[0, -1, :c], normalize=False)
                if opt.use_instance:
                    edges = util.tensor2im(
                        real_A[0, -1, -1:, ...], normalize=False)
                    input_image += edges[:, :, np.newaxis]

                if opt.add_face_disc:
                    ys, ye, xs, xe = modelD.module.get_face_region(
                        real_A[0, -1:])
                    if ys is not None:
                        input_image[ys, xs:xe, :] = input_image[ye, xs:xe,
                                                                :] = input_image[ys:ye, xs, :] = input_image[ys:ye, xe, :] = 255

                visual_list = [('input_image', input_image),
                               ('fake_image', util.tensor2im(fake_B[0, -1])),
                               ('fake_first_image', util.tensor2im(fake_B_first)),
                               ('fake_raw_image', util.tensor2im(
                                   fake_B_raw[0, -1])),
...
def get_skipped_frames(B_all, B, t_scales, tD):
    ...
    for s in range(t_scales):
        ...
        tDs = tD ** s
        # number of frames the final triplet frames span before skipping (e.g., 2, 6, 18, ...)
        span = tDs * (tD-1)
        ...
        if n_groups > 0:
            for t in range(0, n_groups, tD):
                ...
                B_skipped[s] = torch.cat(
                    [B_skipped[s], skip]) if B_skipped[s] is not None else skip
...
def get_skipped_flows(flowNet, flow_ref_all, conf_ref_all, real_B, flow_ref, conf_ref, t_scales, tD):
    ...
    if flow[0] is not None:
        flow_ref_skipped[0], conf_ref_skipped[0] = flow[0][:,
                                                           1:], conf[0][:, 1:]

    for s in range(1, t_scales):
        if real_B[s] is not None and real_B[s].size()[1] == tD:
            flow_ref_skipped[s], conf_ref_skipped[s] = flowNet(
                real_B[s][:, 1:], real_B[s][:, :-1])
...
def get_skipped_frames_sparse(B_all, B, t_scales, tD, n_frames_load, i, is_flow=False):
    ...
    for s in range(t_scales):
        ...
        if t_len > 0 and (t_len % tD) == 0:
            # get rid of unnecessary past frames
            B_all[s] = B_all[s][:, (-tD+1):]

        if s == 0:
            B_all[0] = torch.cat([B_all[0].detach(), B],
                                 dim=1) if B_all[0] is not None else B
        else:
            tDs = tD ** s
            ...
            if idx_start < n_frames_load:
                ...
                B_all[s] = torch.cat(
                    [B_all[s].detach(), tmp], dim=1) if B_all[s] is not None else tmp
