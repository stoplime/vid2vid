...
class Vid2VidModelD(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if opt.batchSize == 1:
            gpu_split_id += 1
        self.gpu_ids = (opt.gpu_ids[gpu_split_id:] + [opt.gpu_ids[0]]
        ...
        if opt.continue_train or opt.load_pretrain:
            self.load_network(
                self.netD, 'D', opt.which_epoch, opt.load_pretrain)
            for s in range(opt.n_scales_temporal):
                self.load_network(getattr(self, 'netD_T'+str(s)),
                                  'D_T'+str(s), opt.which_epoch, opt.load_pretrain)
            if opt.add_face_disc:
                self.load_network(self.netD_f, 'D_f',
        ...
        if opt.add_face_disc:
            self.loss_names += ['G_f_GAN',
                                'G_f_GAN_Feat', 'D_f_real', 'D_f_fake']
        ...
        for s in range(opt.n_scales_temporal):
            ...
            optimizer_D_T = torch.optim.Adam(
    ...
    def compute_loss_D(self, netD, real_A, real_B, fake_B):
        ...
        loss_G_GAN, loss_G_GAN_Feat = self.GAN_and_FM_loss(
            pred_real, pred_fake)
    ...
    def compute_loss_D_T(self, real_B, fake_B, flow_ref, conf_ref, scale_T):
        ...
        real_B = real_B.view(-1, self.output_nc * self.tD,
                             self.height, self.width)
        fake_B = fake_B.view(-1, self.output_nc * self.tD,
                             self.height, self.width)
        if flow_ref is not None:
            flow_ref = flow_ref.view(-1, 2 * (self.tD-1),
                                     self.height, self.width)
        ...
        loss_G_T_GAN, loss_G_T_GAN_Feat = self.GAN_and_FM_loss(
            pred_real, pred_fake)
    ...
    def forward(self, scale_T, tensors_list):
        ...
        if scale_T > 0:
            ...
            loss_list = [loss_G_T_GAN, loss_G_T_GAN_Feat,
                         loss_D_T_real, loss_D_T_fake, loss_G_T_Warp]
        ...
        if flow is not None:
            ...
            loss_F_Warp = self.criterionFlow(
                real_B_warp, real_B, conf_ref) * lambda_T
        ...
        #################### fake_B loss ####################
        # VGG + GAN loss
        loss_G_VGG = (self.criterionVGG(fake_B, real_B) *
        ...
        loss_G_Warp = self.criterionWarp(
            fake_B, fake_B_warp_ref.detach(), conf_ref) * lambda_T
        ...
        if self.opt.add_face_disc:
            loss_list += [loss_G_f_GAN, loss_G_f_GAN_Feat,
                          loss_D_f_real, loss_D_f_fake]
        loss_list = [loss.unsqueeze(0) for loss in loss_list]
        return loss_list
    ...
    def get_face_region(self, real_A):
        ...
        if not self.opt.openpose_only:
            ...
            face = ((real_A[:, 0] > 0.19) & (real_A[:, 0] < 0.21) & (
                real_A[:, 1] < -0.99) & (real_A[:, 2] > -0.61) & (real_A[:, 2] < -0.59)).nonzero()
        if face.size()[0]:
            y, x = face[:, 1], face[:, 2]
    ...
    def save(self, label):
        ...
        for s in range(self.opt.n_scales_temporal):
            self.save_network(getattr(self, 'netD_T'+str(s)),
                              'D_T'+str(s), label, self.gpu_ids)
        if self.opt.add_face_disc:
