...
class Vid2VidModelG(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.split_gpus = (self.opt.n_gpus_gen < len(
            self.opt.gpu_ids)) and (self.opt.batchSize == 1)
        ...
        if not self.isTrain or opt.continue_train or opt.load_pretrain:
            for s in range(self.n_scales):
                self.load_network(getattr(self, 'netG'+str(s)),
                                  'G'+str(s), opt.which_epoch, opt.load_pretrain)

        self.netG_i = self.load_single_G() if self.use_single_G else None

        # define training variables
        if self.isTrain:
            # number of gpus for running generator
            self.n_gpus = self.opt.n_gpus_gen // self.opt.batchSize
            # number of frames to backpropagate the loss
            self.n_frames_bp = 1
            # number of frames in each GPU
            self.n_frames_per_gpu = min(
                self.opt.max_frames_per_gpu, self.opt.n_frames_total // self.n_gpus)
            self.n_frames_load = self.n_gpus * \
                self.n_frames_per_gpu   # number of frames in all GPUs
            if self.opt.debug:
        ...
        if self.isTrain:
            ...
            if not self.finetune_all:
                print('------------ Only updating the finest scale for %d epochs -----------' %
                      opt.niter_fix_global)

            # initialize optimizer G
            params = list(
                getattr(self, 'netG'+str(self.n_scales-1)).parameters())
            ...
            self.optimizer_G = torch.optim.Adam(
                params, lr=lr, betas=(beta1, beta2))
    ...
    def encode_input(self, input_map, real_image, inst_map=None):
        ...
        if self.opt.label_nc != 0:
            ...
            oneHot_size = (self.bs, tG, self.opt.label_nc,
    ...
    def forward(self, input_A, input_B, inst_A, fake_B_prev):
        ...
        for s in range(self.n_scales):  # broadcast netG to all GPUs used for generator
            ...
            netG_s = torch.nn.parallel.replicate(
                netG_s, self.opt.gpu_ids[:gpu_split_id]) if self.split_gpus else [netG_s]
            netG.append(netG_s)
    ...
    def generate_frame_train(self, netG, real_A_all, fake_B_pyr, start_gpu, is_first_frame):
        ...
        for t in range(n_frames_load):
            # the GPU idx where we generate this frame
            gpu_id = (t // self.n_frames_per_gpu +
                      start_gpu) if self.split_gpus else start_gpu
            # the GPU idx where the net is located
            net_id = gpu_id if self.split_gpus else 0
            ...
            for s in range(n_scales):
                ...
                real_As_reshaped = real_As[:, t:t +
                                           tG, ...].view(self.bs, -1, h, w).cuda(gpu_id)
                ...
                mask_F = self.compute_mask(
                    real_As, t+tG-1) if self.opt.fg else None
                ...
                fake_B_pyr[si] = self.concat(
                    [fake_B_pyr[si], fake_B.unsqueeze(1).cuda(dest_id)], dim=1)
                if s == n_scales-1:
                    fake_Bs_raw = self.concat(
                        [fake_Bs_raw, fake_B_raw.unsqueeze(1).cuda(dest_id)], dim=1)
                    if flow is not None:
                        flows = self.concat(
                            [flows, flow.unsqueeze(1).cuda(dest_id)], dim=1)
                        weights = self.concat(
                            [weights, weight.unsqueeze(1).cuda(dest_id)], dim=1)

        return fake_B_pyr, fake_Bs_raw, flows, weights

    def inference(self, input_A, input_B, inst_A):
        with torch.no_grad():
            real_A, real_B, pool_map = self.encode_input(
                input_A, input_B, inst_A)
            self.is_first_frame = not hasattr(
                self, 'fake_B_prev') or self.fake_B_prev is None
            if self.is_first_frame:
                self.fake_B_prev = self.generate_first_frame(
                    real_A, real_B, pool_map)

            real_A = self.build_pyr(real_A)
            self.fake_B_feat = self.flow_feat = self.fake_B_fg_feat = None
            for s in range(self.n_scales):
                fake_B = self.generate_frame_infer(
                    real_A[self.n_scales-1-s], s)
    ...
    def generate_frame_infer(self, real_A, s):
        ...
        self.fake_B_prev[si] = torch.cat(
            [self.fake_B_prev[si][1:, ...], fake_B])
        return fake_B

    def generate_first_frame(self, real_A, real_B, pool_map=None):
        tG = self.opt.n_frames_G
        if self.opt.no_first_img:          # model also generates first frame
            fake_B_prev = Variable(self.Tensor(
                self.bs, tG-1, self.opt.output_nc, self.height, self.width).zero_())
        elif self.opt.isTrain or self.opt.use_real_img:  # assume first frame is given
            fake_B_prev = real_B[:, :(tG-1), ...]
        # use another model (trained on single images) to generate first frame
        elif self.opt.use_single_G:
            fake_B_prev = None
            if self.opt.use_instance:
                real_A = real_A[:, :, :self.opt.label_nc, :, :]
            for i in range(tG-1):
                feat_map = self.get_face_features(
                    real_B[:, i], pool_map[:, i]) if self.opt.dataset_mode == 'face' else None
                fake_B = self.netG_i.forward(
                    real_A[:, i], feat_map).unsqueeze(1)
                fake_B_prev = self.concat([fake_B_prev, fake_B], dim=1)
        else:
            raise ValueError(
                'Please specify the method for generating the first frame')
    ...
    def build_pyr(self, tensor, nearest=False):  # build image pyramid from a single image
        ...
        for s in range(1, self.n_scales):
            ...
            down = downsample(tensor[-1].view(-1, h, w)
                              ).view(b, t, c, h//2, w//2)
            tensor.append(down)
        return tensor
        ...
        if 'City' in self.opt.dataroot:
            ...
            if opt.loadSize == 512:
                ...
                netG = networks.define_G(
                    35, 3, 0, 64, 'global', 3, 'instance', 0, self.gpu_ids, opt)
            elif opt.loadSize == 1024:
                load_path = single_path + 'latest_net_G_1024.pth'
                netG = networks.define_G(
                    35, 3, 0, 64, 'global', 4, 'instance', 0, self.gpu_ids, opt)
            elif opt.loadSize == 2048:
                load_path = single_path + 'latest_net_G_2048.pth'
                netG = networks.define_G(
                    35, 3, 0, 32, 'local', 4, 'instance', 0, self.gpu_ids, opt)
            else:
            ...
            opt.feat_num = 16
            netG = networks.define_G(
                15, 3, 0, 64, 'global_with_features', 3, 'instance', 0, self.gpu_ids, opt)
            encoder_path = single_path + 'latest_net_E.pth'
            self.netE = networks.define_G(
                3, 16, 0, 16, 'encoder', 4, 'instance', 0, self.gpu_ids)
    ...
    def get_face_features(self, real_image, inst):
        ...
        # feature map for test img (for each facial part)
        feat_ori = torch.FloatTensor(7, self.opt.feat_num, 1)
        # feature map for training imgs
        feat_ref = torch.FloatTensor(7, self.opt.feat_num, num_images)
        for label in np.unique(inst_np):
            idx = (inst == int(label)).nonzero()
            for k in range(self.opt.feat_num):
                feat_ori[label, k] = float(
                    feat_map[idx[0, 0], idx[0, 1] + k, idx[0, 2], idx[0, 3]])
                for m in range(num_images):
                    feat_ref[label, k, m] = features[label][m, k]
        cluster_idx = self.dists_min(feat_ori.expand_as(
            feat_ref).cuda(), feat_ref.cuda(), num=1)
        ...
        for label in np.unique(inst_np):
            ...
            idx = (inst == int(label)).nonzero()
            for k in range(self.opt.feat_num):
                feat_map[idx[:, 0], idx[:, 1] + k, idx[:, 2], idx[:, 3]
                         ] = feat[min(cluster_idx, feat.shape[0]-1), k]
        ...
        if tensors[0] is not None and tensors[1] is not None:
            if isinstance(tensors[0], list):
                ...
                for i in range(len(tensors[0])):
                    tensors_cat.append(self.concat(
                        [tensors[0][i], tensors[1][i]], dim=dim))
                return tensors_cat
    ...
    def save(self, label):
        for s in range(self.n_scales):
            self.save_network(getattr(self, 'netG'+str(s)),
                              'G'+str(s), label, self.gpu_ids)

    def update_learning_rate(self, epoch):
        lr = self.opt.lr * (1 - (epoch - self.opt.niter) /
    ...
    # finetune all scales instead of just finest scale
    def update_fixed_params(self):
        params = []
        for s in range(self.n_scales):
            params += list(getattr(self, 'netG'+str(s)).parameters())
        self.optimizer_G = torch.optim.Adam(
            params, lr=self.old_lr, betas=(self.opt.beta1, 0.999))
        self.finetune_all = True
        print('------------ Now finetuning all scales -----------')

    # increase number of backpropagated frames and number of frames in each GPU
    def update_training_batch(self, ratio):
        ...
        if nfb < nfl:
            ...
            print('-------- Updating number of backpropagated frames to %d ----------' %
                  self.n_frames_bp)

        if self.n_frames_per_gpu < self.opt.max_frames_per_gpu:
            self.n_frames_per_gpu = min(
                self.n_frames_per_gpu*2, self.opt.max_frames_per_gpu)
            self.n_frames_load = self.n_gpus * self.n_frames_per_gpu
            print('-------- Updating number of frames per gpu to %d ----------' %
